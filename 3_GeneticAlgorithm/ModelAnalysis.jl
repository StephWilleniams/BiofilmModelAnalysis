module ModelAnalysis  # Name of module
...
# Generate synthetic data from the population solution
function SyntheticPopulation_fixIVP(sol, dist, N_synthetics)
    synthetic_data = zeros(size(sol,1), size(sol,2), N_synthetics)
    N_species = size(sol,1)
    N_times = size(sol,2)
    for i = 1:N_species
        synthetic_data[i,1,:] .= sol[i,1]
        for n = 1:N_synthetics
            synthetic_data[i,2:N_times,n] = sol[i,2:N_times] .* rand(dist,N_times-1)
        end
    end
    return synthetic_data
end
...
function PLB(index_of_interest, parameter_range_of_interest, n_PLs, ode_model, solver_algorithm, u0, dist, organ::organism, fixed_paramaters, initial_varying_params, varying_indices, synthetic_data)
    ...
    return output, root1, root2, abs(root1 - root2)
end
...
function CloneOrganisms(organisms::Vector{organism}, sig, dt, t_start, t_end)
    ...
    for i in 1:length(organisms)
        ...
        for j in 2:length(organisms[i].schedule)
            organisms[i].schedule[j] = min(max(organisms[i].schedule[j],dt),t_end-dt)
...
function PrintOrganisms(organisms::Vector{organism},io,write_outout)
    if write_outout
        println(io,"Organisms as follows:")
        for organ in organisms
            println(io,"Schedule: ", organ.schedule)
            println(io,"Ranking: ", organ.ranking)
            println(io,"Fitness: ", organ.fitness)
            println(io," ")
        end
    else
...
function SaveBestOrganism(organisms::Vector{organism})
    filename = "best_organism.csv"
    best_index = findfirst(org -> org.ranking == 1, organisms)
    df = DataFrame(organisms[best_index], :auto)
    CSV.write(filename, df)
end
...
SolvePopulation, SyntheticPopulation, SyntheticPopulation_fixIVP,
...
end # End of module ModelAnalysis